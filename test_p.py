...
def pre_train(agent,dict_pre_training):
    ...
    for stock in tqdm(dict_pre_training.keys()):
        for i in tqdm(range(dict_pre_training[stock])):
            ...
            data = np.load("./data_martingale/" + stock + "/p_" + str(i) + ".npy")
...
def main():
    ...
    plt.show()
    '''
    return statistics.mean(mean_list),statistics.mean(performance_list),learned_strategy
