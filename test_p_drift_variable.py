...
def pre_train(agent,dict_pre_training):
    ...
    for stock in tqdm(dict_pre_training.keys()):
        for i in tqdm(range(dict_pre_training[stock])):
            ...
            data = np.load("./data_6_drift_updown/" + stock + "/p_" + str(i) + ".npy")
...
#test AC strategy 
    ...
    '''
    return mean_list_TWAP, mean_list_AC
...
def main():
    ...
    mean_list_TWAP,mean_list_AC=test(agent)
    #make plots
...
if __name__ == "__main__":
    ...
    print(mean_list_TWAP,mean_list_AC)
    print('medie di performance')
    print(statistics.mean(mean_list_TWAP),statistics.mean(mean_list_AC))
